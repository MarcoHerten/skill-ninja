...
- **`/ninja profile`** *(live, v1.3 — store-side since v1.5)* — named skill sets per purpose ("the content setup", "the code setup"): `save` a member list, `apply` it in a repo (project-local symlinks, additive on the global baseline), `lift` it again. Lives at `<store>/profiles.json` and travels with the store repo ([ADR-0017](./docs/adr/0017-collections-and-profiles-travel-with-the-store.md)).
- **`/ninja collection`** *(live, v1.3.1 — store-side since v1.5)* — named, personal filters over the inventory ("everything from Nils"): pattern lists at `<store>/collections.json`, resolved live by `cat @<name>`, `find @<name>`, the page's dropdown, and `off/manual/on --collection`. The owner's view, never data on the skills — and traveling with the store repo ([ADR-0017](./docs/adr/0017-collections-and-profiles-travel-with-the-store.md)).
