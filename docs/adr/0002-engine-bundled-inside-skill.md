...
The Node engine lives **inside the skill** at `skills/ninja/engine/`, not at the repo root.

## Decision

The skill ships as `skills/ninja/SKILL.md` (the interface layer) **and** its deterministic Node engine at `skills/ninja/engine/` (`cli.js`, `config.js`, `agents.js`). The `package.json` `bin` points at the engine entry; the `SKILL.md` runs the engine via `node <SKILL_DIR>/engine/cli.js <command>`.
...
- Engine code is developed under `skills/ninja/engine/`; later tickets (init/status/doctor/add/diff) add modules there.
