...
| `/ninja ingest`| `ingest`        | Bulk-analyze a messy source directory (skills in any packaging, prompt documents), report the proposed resolution, and on `--apply` store the winners with provenance — read-only on the source, links nothing. | **Dry run live (v1.1; `--apply` planned)** |
