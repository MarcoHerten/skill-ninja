...
| -------------------- | --------------- | --------------------------------------------------------------------------- | ------------- |
| `/ninja init`  | `init`          | Analyze the machine; scan agent roots, vaults, and project dirs, write the cached inventory. `--store <name\|path>` selects the canonical store. | **Live (v1.4: `--store`)** |
...
### `/ninja profile` (live, v1.3)

Named, reusable skill sets applied per project (ADR-0014) — "these skill setups in this repo because I produce content here". Runs `node <SKILL_DIR>/engine/cli.js profile <sub>` and relays the output:
