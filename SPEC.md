...
### Plugin awareness (v1.6)
51. As a user, I want skills bundled inside agent plugins (the Agent Plugins 1.0.0 layout, and the pre-spec plugin caches) to appear in `status`, `page`, `cat`, and `find` as plugin-owned — attributed to their plugin — so the inventory shows where every skill lives, including the plugin channel.
...
54. As a user, I want a copy button directly behind each skill name on the status page, so I can copy the skill's name and paste it into any chat or terminal to invoke the skill there. (Revised 2026-08-19: copies the name, not the full SKILL.md — the full-file dump buried the one token I wanted to paste. Amended 2026-08-21: the token leads with the slash — `/name` — so a paste into the agent chat invokes the skill directly; a terminal run drops the slash.)
...
- **Plugin caches are audited (ADR-0018).** Each configured agent's plugin cache (`~/.claude/plugins/cache`, `~/.zcode/cli/plugins/cache` — the map grows with Agent Plugins 1.0.0 adoption) is a scan root of its own. A plugin manifest (`plugin.json` per the spec, or the pre-spec `.claude-plugin/plugin.json` / `.zcode-plugin-seed.json` / `package.json`) bounds a plugin: only its `skills/` subtree is walked, so non-skill plugin content never surfaces. Bundled skills are **plugin-owned** — shown everywhere, touched nowhere: `doctor` proposes no repair, availability switches refuse, and an all-plugin version spread is not a duplicate.
